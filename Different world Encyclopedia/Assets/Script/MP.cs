...
public class MP : MonoBehaviour
{
    ...
    public void initHPValue(float fullHP, float currentHP)
    {
        ...
        this.MPGAGE.GetComponent<Image>().fillAmount = CURRENT_HP_VALUE / FULL_HP_VALUE;
    }
    ...
    public void gageController()
    {
        if (IS_GAGE_ANIMATION_ON)
        {
            ...
            {
                if (TARGET_HP_VALUE / FULL_HP_VALUE <= this.MPGAGE.GetComponent<Image>().fillAmount)
                    this.MPGAGE.GetComponent<Image>().fillAmount -= 0.01f;
                else
