...
public class HP : MonoBehaviour
{
    ...
    public void initHPValue(float fullHP, float currentHP)
    {
        ...
        this.HPGAGE.GetComponent<Image>().fillAmount = CURRENT_HP_VALUE / FULL_HP_VALUE;
    }
    ...
    public void gageController()
    {
        if (IS_GAGE_ANIMATION_ON)
        {
            ...
            {
                if (TARGET_HP_VALUE / FULL_HP_VALUE <= this.HPGAGE.GetComponent<Image>().fillAmount)
                    this.HPGAGE.GetComponent<Image>().fillAmount -= 0.01f;
                else
